...
class MPPIKSOS(SamplingBasedController):
    ...
    def update_params(
        ...
        def _do_ksos(ops):
            ...
            ksos_mean = io_callback(  # type: ignore[arg-type]
            ...
            ksos_best_costs = self.get_cost_with_best_samples(
                params.states, params, params.tk, ksos_mean
            )
            mppi_best_costs = self.get_cost_with_best_samples(
                params.states, params, params.tk, mppi_mean
            )
        ...
        update_mean = jax.lax.cond(predicate, _do_ksos, _keep, operand=(new_iter, mppi_mean))

        # jax.debug.print("update_mean (every10) {}", update_mean)

        # use update_mean (may be equal to mean when predicate is false)
        return params.replace(mean=update_mean, iter=new_iter)
